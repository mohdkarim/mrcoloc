...
ST4 <- bind_rows(rs_l2g, rs_pqtl_l2g, rs_pqtl, rs_other, rs_mrtype, rs_mrcoloc) %>%
  ...
  select(panel_group = label, source_label = source, rs_estimate = est,
         rs_lwr_95ci = lwr.ci, rs_upr_95ci = upr.ci, count_string = n)
...
Key <- tribble(
  ...
  "COLUMN DEFINITIONS", "--- ST1: Proteomic_GWAS ---", NA,
...
sheet_titles <- list(
  ...
  "ST1 - Proteomic_GWAS" = "Supplementary Table 1: Proteomic GWAS Datasets",
  "ST2 - Outcome_GWAS" = "Supplementary Table 2: Outcome GWAS Datasets",
...
for (sheet_name in names(all_tables_clean)) {
  ...
  if (sheet_name == "Key") {
    ...
    # Special handling for ST16 - limit width of text-heavy columns
    if (sheet_name == "ST16 - All_MR_pairs") {
      # Set max widths for columns with long comma-separated lists
      text_heavy_cols <- c("data_sources", "outcomes", "outcome_trait", "trait_key_term", 
                          "snp_ciscoloc_all", "snp_transcoloc_all", "trans_coloc_genes_all", 
                          "pav_terms_all", "triangulation", "mr_coloc_types")
      col_indices <- which(names(df) %in% text_heavy_cols)
      if (length(col_indices) > 0) {
        setColWidths(wb, sheet_name, cols = col_indices, widths = 50)
      }
    }
